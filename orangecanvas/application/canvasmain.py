...
class CanvasMainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        (self.zoom_in_action, self.zoom_out_action, self.zoom_reset_action,
         ...
         self.canvas_text_action, self.canvas_arrow_action, self.canvas_auto_arrow_action) = tool_actions # TigerODM
        ...
        self.canvas_auto_arrow_action.setIcon(load_styled_svg_icon("autoArrow.svg", self.canvas_toolbar)) # TigerODM
        ...
        dock_actions = [
            ...
            self.canvas_auto_arrow_action, # TigerODM
            self.freeze_action,
    ...
    def setup_actions(self):
        ...
        if builtins.IsTiger:
            self.get_started_action = QAction(
                "Lentilles", self,
                objectName="welcome-lentilles",
                toolTip="Lance le workflow d'application Lentilles",
                icon=load_styled_svg_icon("lentilles.svg"),
                triggered=self.start_lentilles,
            )

            self.get_started_screencasts_action = QAction(
                "TigerChat", self,
                objectName="welcome-tigerchat",
                toolTip="Lance TigerChat",
                icon=load_styled_svg_icon("Tiger.svg"),
                triggered=self.start_tiger_chat,
            )
            ...
            self.documentation_action = QAction(
                ...
                # On utilise lambda pour dire : "N'exécute ça que quand on clique"
                triggered=lambda: QDesktopServices.openUrl(QUrl("https://www.tigerodm.com/public/documentation/")),
            )

            self.examples_action = QAction(
                "AppWorkflows", self,
                objectName="welcome-appworkflows",
                toolTip="Ouvre les workflows d'application.",
                icon=load_styled_svg_icon("Examples.svg"),
                triggered=self.app_workflows,
            )
            ...
            self.get_started_action = QAction(
                ...
                toolTip="View a 'Get Started' introduction.",
                icon=load_styled_svg_icon("Documentation.svg")
            )
            ...
            self.documentation_action = QAction(
                "Documentation", self,
                objectName="documentation-action",
                toolTip="View reference documentation.",
                icon=load_styled_svg_icon("Documentation.svg"),
            )
            config_url_action(self.documentation_action,  "Documentation")

            self.examples_action = QAction(
                "Example Workflows", self,
                objectName="examples-action",
                toolTip="Browse example workflows.",
        ...
        # TigerODM actions
        self.tiger_chat_action = QAction(
            "TigerChat", self,
            objectName="tiger-chat",
            toolTip="Lance TigerChat.",
            triggered=self.start_tiger_chat,
        )
        self.lentilles_action = QAction(
            "Lentilles", self,
            objectName="lentilles",
            toolTip="Lance le workflow d'application Lentilles.",
            triggered=self.start_lentilles,
        )
        self.show_mwe_action = QAction(
            "Load MWE", self,
            objectName="show-mwe",
            toolTip="Load basic workflows to start manipulating AI functionalities.",
            triggered=self.show_mwe,
        )
        self.my_workflows_action = QAction(
            "Load my workflows", self,
            objectName="my-workflows",
            toolTip="Load the workflows from your AAIT Store.",
            triggered=self.my_workflows,
        )
        self.app_workflows_action = QAction(
            "Load AppWorkflows", self,
            objectName="app-workflows",
            toolTip="Load the Application workflows from your AAIT Store.",
            triggered=self.app_workflows,
        )
        self.tutorial_workflows_action = QAction(
            "Load Tutorial Workflows", self,
            objectName="tutorial-workflows",
        ...
        self.key_manager_action = QAction(
            "Key Manager", self,
            objectName="key-manager",
            toolTip="Open the key manager to handle passwords and ID",
            triggered=self.open_key_manager,
        )
        self.link_html_workflow_manager_action = QAction(
            "Link HTML Workflow Manager", self,
            objectName="link-html-workflow-manager",
            toolTip="Open the link html workflow manager to handle json configuration",
            triggered=self.open_link_html_workflow_manager,
        )
        self.tiger_notepad_action = QAction(
            "TigerNotepad", self,
            objectName="tiger-notepad",
            toolTip="Open the TigerNotepad",
            triggered=self.open_tiger_notepad,
        )

        # # Not implemented yet
        # self.configure_store_action = QAction(
        #     "Configure AAIT store", self,
        #     objectName="configure-store",
        #     toolTip="Configure your AAIT store to easily load and save workflows & models.",
        #     triggered=self.configure_store,
        # )
        # Not implemented yet
        self.run_tests_action = QAction(
            "Verify install", self,
            objectName="run-tests",
            toolTip="Run tests to verify if your TigerODM install is damaged.",
            triggered=self.run_tests,
        )



        # Action group for recent scheme actions
    ...
    def show_mwe(self):
        ...
        mwe_path = os.path.join(local_store_path, "MWE")
        os.makedirs(mwe_path, exist_ok=True)
        self.mwe_dialog(mwe_path)
    ...
    def app_workflows(self):
        try:
            from orangecontrib.AAIT.utils.MetManagement import get_local_store_path
            local_store_path = get_local_store_path()
            appwf_path = os.path.join(local_store_path, "AppWorkflows")
            os.makedirs(appwf_path, exist_ok=True)
            self.mwe_dialog(appwf_path)
        except Exception as e:
            import logging
            logging.error(f"Failed to open appWorkflows: {e}")


    def tutorial_workflows(self):
        try:
            from orangecontrib.AAIT.utils.MetManagement import get_local_store_path
            local_store_path = get_local_store_path()
            tutowf_path = os.path.join(local_store_path, "Tutorial")
            os.makedirs(tutowf_path, exist_ok=True)
            self.mwe_dialog(tutowf_path)
        except Exception as e:
            import logging
            logging.error(f"Failed to open Tutorial workflows: {e}")

    def open_key_manager(self):
        try:
            from orangecontrib.IO4IT.toolViews.key_manager_ui import KeyManagerUI
            dlg = KeyManagerUI(self)
            dlg.setAttribute(Qt.WA_DeleteOnClose)
            dlg.show()
        except Exception as e:
            # Using Orange's logger is better than a raw raise in GUI
            import logging
            logging.error(f"Failed to open Key Manager: {e}")

    def open_link_html_workflow_manager(self):
        try:
            from  orangecontrib.IO4IT.toolViews.linkHTLMWorkflow_manager_ui import LinkWorkflowManagerUI
            dlg = LinkWorkflowManagerUI(self)
            dlg.setAttribute(Qt.WA_DeleteOnClose)
            dlg.show()
        except Exception as e:
            # Using Orange's logger is better than a raw raise in GUI
    ...
    def open_aait_ressource_manager(self):
        try:
            from orangecontrib.AAIT.utils.OWAAITResourcesManager import OWAAITResourcesManager
            # Check if it's already open to avoid multiple instances
            if not hasattr(self, "_aait_ressource_manager") or self._aait_ressource_manager is None:
                # Pass 'self' so it stays linked to the Canvas window
                self._aait_ressource_manager = OWAAITResourcesManager(self)
            self._aait_ressource_manager.show()
            self._aait_ressource_manager.raise_()
            self._aait_ressource_manager.activateWindow()
        except Exception as e:
            # Using Orange's logger is better than a raw raise in GUI
            import logging
            logging.error(f"Failed to open aait ressource manager: {e}")


    def open_tiger_notepad(self):
        try:
            from orangecontrib.AAIT.utils.tools.TigerODM_notepad import EditorMainWindow
            # Check if it's already open to avoid multiple instances
            if not hasattr(self, "_tiger_notepad") or self._tiger_notepad is None:
                # Pass 'self' so it stays linked to the Canvas window
                self._tiger_notepad = EditorMainWindow(self)
            self._tiger_notepad.show()
            self._tiger_notepad.raise_()
            self._tiger_notepad.activateWindow()
        except Exception as e:
            # Using Orange's logger is better than a raw raise in GUI
            import logging
            logging.error(f"Failed to open TigerODM Notepad: {e}")

    def start_tiger_chat(self):
        try:
            from orangecontrib.AAIT.utils.MetManagement import get_local_store_path
            local_store_path = get_local_store_path()
            tiger_chat_path = os.path.join(local_store_path, "AppWorkflows", "RAG_LeTigre.ows")
            self.open_scheme_file(tiger_chat_path)
        except Exception as e:
            import logging
            logging.error(f"Failed to open TigerChat: {e}")

    def start_lentilles(self):
        try:
            from orangecontrib.AAIT.utils.MetManagement import get_local_store_path
            local_store_path = get_local_store_path()
            tiger_chat_path = os.path.join(local_store_path, "AppWorkflows", "lentilles.ows")
            self.open_scheme_file(tiger_chat_path)
        except Exception as e:
            import logging
            logging.error(f"Failed to open Lentilles: {e}")



    def run_tests(self):
        # TODO
        print("RUNING SOME TESTS BABY YEEEH !!")
    ...
    def setup_menu(self):
        ...
        if builtins.IsTiger:
            # TigerODM menu
            self.tigerODM_menu = QMenu(
                "TigerODM", menu_bar, objectName="tigerODM-menu",
            )
        else:
            # TigerODM menu
            self.tigerODM_menu = QMenu(
                "Tools", menu_bar, objectName="tigerODM-menu",
            )

        if builtins.IsTiger:
            self.tigerODM_menu.addActions([
                self.tiger_chat_action,
                self.lentilles_action,
            ])

            self.tigerODM_menu.addSeparator()

            self.tigerODM_menu.addActions([
                self.app_workflows_action,
        ...
        self.tigerODM_menu.addActions([
            ...
            self.key_manager_action,
            self.link_html_workflow_manager_action,
    ...
    def mwe_dialog(self, directory):
        ...
        """
        #directory = os.path.join(os.path.dirname(os.path.dirname(sys.executable)), "aait_store", "Workflow_store_mwes")
        ows_files = [os.path.join(directory, f) for f in os.listdir(directory) if f.endswith(".ows")]
        tutors = [examples.ExampleWorkflow(resource=ows_file) for ows_file in ows_files]
    ...
    def welcome_dialog(self):
        ...
        bottom_row = [self.get_started_action, self.examples_action,
                      self.documentation_action]
