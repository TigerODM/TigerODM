...
from AnyQt.QtCore import (
    Qt, QObject, QEvent, QSignalMapper, QCoreApplication, QPointF, QRectF, QLineF,
    QMimeData, Slot)
...
class SchemeEditWidget(QWidget):
    ...
    def __setupActions(self):
        ...
        # TigerODM
        self.__autoArrowAnnotationAction = QAction(
            "autoArrow", self,
            objectName="auto-arrow-action",
            toolTip="Add arrows & text on top of widgets group (requires alignment)",
            checkable=False,
            triggered=self.__draw_group_arrows,
        )
        ...
        self.addActions([
            ...
            self.__autoArrowAnnotationAction, # TigerODM
    ...
    # TigerODM
    def __draw_group_arrows(self):
        # 1. Get all nodes and group them by their Y coordinate
        nodes = self.__scheme.nodes
        y_groups = defaultdict(list)
        for node in nodes:
            # node.position is (x, y)
            y_groups[node.position[1]].append(node)

        for y, row_nodes in y_groups.items():
            # Sort widgets in this row from left to right
            row_nodes.sort(key=lambda n: n.position[0])

            # 2. Identify chains spaced by exactly 150
            current_group = []
            for i, node in enumerate(row_nodes):
                if not current_group:
                    current_group.append(node)
                else:
                    # Check distance from the previous node in the chain
                    prev_node = current_group[-1]
                    dist = node.position[0] - prev_node.position[0]

                    if dist == 150:
                        current_group.append(node)
                    else:
                        # Chain broken: process the finished group and start new one
                        if len(current_group) > 1:  # Only draw if it's a "group"
                            self.__create_group_arrow(current_group)
                        current_group = [node]

            # Process the last group in the row
            if len(current_group) > 1:
                self.__create_group_arrow(current_group)

    # TigerODM
    def __create_group_arrow(self, group_nodes):
        # 1. Calcul des coordonnées cibles
        first_x = group_nodes[0].position[0]
        last_x = group_nodes[-1].position[0]
        y_center = group_nodes[0].position[1]

        arrow_y = y_center - 42
        start_pt = (first_x - 20, arrow_y)
        end_pt = (last_x + 30, arrow_y)

        # 2. Vérification de l'existence (Dédoublonnage)
        # On regarde dans le schéma si une annotation identique existe déjà
        for existing_annot in self.__scheme.annotations:
            # On vérifie si c'est une flèche et si les points matchent
            if isinstance(existing_annot, SchemeArrowAnnotation):
                if existing_annot.start_pos == start_pt and existing_annot.end_pos == end_pt:
                    # PAF ! Déjà là, on s'arrête ici.
                    return

        # 3. Si on arrive ici, c'est qu'elle n'existe pas -> Création
        color = "#39B54A"
        annot = SchemeArrowAnnotation(start_pt, end_pt)
        annot.set_color(color)

        arrow_item = self.__scene.add_annotation(annot)

        # Mise à jour visuelle
        p1 = arrow_item.mapFromScene(QPointF(*start_pt))
        p2 = arrow_item.mapFromScene(QPointF(*end_pt))
        arrow_item.setLine(QLineF(p1, p2))

        # Texte et sauvegarde
        self.__create_text_annotation(start_pt, end_pt, content="Widget Group")
        self.__scheme.add_annotation(annot)

    # TigerODM
    def __create_text_annotation(self, start_pt, end_pt, content="Widget Group"):
        # 1. Calcul des dimensions cibles (arrondies pour la comparaison)
        arrow_width = end_pt[0] - start_pt[0]
        rect_coords = (round(start_pt[0]), round(start_pt[1] - 35), round(arrow_width), 30)

        # 2. Vérification des doublons
        # On parcourt les annotations existantes dans le schéma
        for existing in self.__scheme.annotations:
            if isinstance(existing, SchemeTextAnnotation):
                # On compare le contenu ET la position (rect)
                # existing.rect est souvent un tuple ou un QRectF selon TigerODM
                if existing.content == content and existing.rect == rect_coords:
                    return  # On ne crée rien, c'est déjà là !

        # 3. Création si non trouvé
        text_annot = SchemeTextAnnotation(rect_coords)
        text_annot.set_content(content)
        text_annot.set_font({"family": "Segoe UI", "size": 16})

        # 4. Ajout visuel
        item = self.__scene.add_annotation(text_annot)
        item.setGeometry(QRectF(*rect_coords))

        # 5. Sauvegarde
        self.__scheme.add_annotation(text_annot)
