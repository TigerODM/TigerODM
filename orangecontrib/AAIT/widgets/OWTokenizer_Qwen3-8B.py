...
class OWTokenizerQwen3_8B(widget.OWWidget):
    name = "Tokenizer - Qwen3 8B"
    description = "Load the tokenizer of Qwen3 8B"
    category = "AAIT - MODELS"
    icon = "icons/qwen-color.png"
    if "site-packages/Orange/widgets" in os.path.dirname(os.path.abspath(__file__)).replace("\\", "/"):
        icon = "icons_dev/qwen-color.png"
    gui = os.path.join(os.path.dirname(os.path.abspath(__file__)), "designer/owtokenizer_qwen3_8b.ui")
    priority=1093
    want_control_area = False

    class Outputs:
        out_model_path = Output("Tokenizer", str, auto_summary=False)
    ...
    def __init__(self):
        ...
        if not os.path.exists(self.model_path):
            if not SimpleDialogQt.BoxYesNo("Model isn't in your computer. Do you want to download it from AAIT store?"):
                return
            try:
                if 0 != GetFromRemote("Tokenizer - Qwen3 8B"):
                    return
            except:
